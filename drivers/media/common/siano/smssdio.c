...
exit_with_error:
	...
	while ((cur_trnsfr == trnsfr_cnt) && (bytes_transfered <= trnsfr_cnt*2))
	{
	...
	kfree(tmp_buf);
	sms_err("total data=%d, total_blocks=%d\n", bytes_transfered, total_blocks);
...
//#define SMS_DEBUG_WORKAROUND
#ifdef SMS_DEBUG_WORKAROUND
static int sms_debug_backup = 0;
#endif
