...
static int smschar_open(struct inode *inode, struct file *file)
{
	...
	if (dev->cdev.dev == MKDEV(smschar_major, smschar_minor)) { //not all device needed, just "mdtvctrl"
		sms_debug("count:%d, dev->coredev:%#x i_ino:%ld\n", atomic_read(&open_count), (unsigned int)dev->coredev, inode->i_ino);
		if ((OPEN_COUNT_INIT + 1) == atomic_inc_return(&open_count)) {
			...
			if (!(dev->coredev)) {
				...
				if (!dev->coredev) {
					...
					sms_err("sms chip wait for plug in times:%d", times);
				}
			}
		}
	} else {
...
static int smschar_release(struct inode *inode, struct file *file)
{
	...
	if (dev->cdev.dev == MKDEV(smschar_major, smschar_minor)) { //not all device needed, just "mdtvctrl"
		int times = 100;
		sms_debug("count:%d i_ino:%ld\n", atomic_read(&open_count), inode->i_ino);
...
sms_info("created nodes for socket %d", j);
	...
	if (host_sms_reset_pin != 0xFFFFFFFF)
	{
		...
		if (gCELLBuffAddr){
			printk("[SMS] gCELLBuffAddr:0x%x\r\n",(unsigned int)gCELLBuffAddr);
		}else{
