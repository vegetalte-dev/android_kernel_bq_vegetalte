...
retry_read:
	...
	if (ret != ARRAY_SIZE(xfer)) {
		if (!retry) {
			...
			dev_err(&client->dev, "%s: i2c transfer failed (%d) ret %d\n",
				__func__, ret, reg);
			return -EIO;
		}
	}
...
static int mxt_handle_pdata(struct mxt_data *data)
{
  ...
  //  rc = mxt_get_dt_coords(data->client->dev, "atmel,panel-coords", data->pdata);
	//if (rc)
		//return rc;

//	rc = mxt_get_dt_coords(data->client->dev, "atmel,display-coords", data->pdata);
	//if (rc)
		//return rc;
