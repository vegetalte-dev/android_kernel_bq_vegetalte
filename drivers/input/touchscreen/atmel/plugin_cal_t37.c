...
static int mxt_surface_aquire_and_compare_auto(struct plugin_cal *p,int sf,int compare,s16 diff_threshold_t9,s16 diff_threshold_t15,int no_wait, unsigned long flag)
{
 ...
 //test bad points purpose
 //#define TEST_BAD_SENSOR
...
/*
static void plugin_cal_t37_hook_t61(struct plugin_cal *p, int id, u8 status)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
	struct t37_observer *obs = p->obs;
	...
	if (id == 0) {
		...
		if (status & MXT_T61_ELAPSED) {
			//if (test_flag(T37_STEP_1_IN | T37_STEP_2_IN,&obs->step)
				set_and_clr_flag(T37_STEP_4_ENTER,T37_STEP_MASK,&obs->step);
		}
	}
}
*/
...
static int mxt_proc_step_1_msg(struct plugin_cal *p,unsigned long pl_flag)
{
	...
	if (test_and_clear_flag(T37_FLAG_RESUME|T37_FLAG_RESET|T37_FLAG_CAL,
		...
		if (test_flag(T37_FLAG_CAL, &flag)) {
			...
			/*
			if(test_flag(T37_FLAG_SUPPRESSED, &flag)) {
				p->set_t6(p->dev, MXT_COMMAND_RESET, 1, false);
			}
			*/
		}
...
static int init_t37_object(struct plugin_cal *p)
{
	...
	cfg->interval_delta_check_end = HZ * 30;//240
	cfg->interval_delta_check_end2 = HZ * 45;//300
	...
	//540s
	//cfg->grad_acc_flag |= WK_KEEP_T80_ALWAYS_ON|BUGS_CHECK_T100_REPORT|BUGS_CHECK_REFERENCE_DROP|BUGS_CHECK_DELTA_HIGH|BUGS_CHECK_REFERENCE_IRON;
...
static int plugin_cal_t37_show(struct plugin_cal *p)
{
	...
	for (i = 0; i < T37_BUFFER_NUMBER; i++) {
		...
		//if (i == T37_REF0 || i == T37_GRAD0_X || i == T37_GRAD0_Y) {
...
struct plugin_cal mxt_plugin_cal_t37 =
{
	...
	//.hook_t61 = plugin_cal_t37_hook_t61,
