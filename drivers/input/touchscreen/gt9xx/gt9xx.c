...
static int goodix_power_init(struct goodix_ts_data *ts)
{
    ...
    if (gpio_is_valid(ts->pdata->power_ldo_gpio)) {
		printk("%s, power_ldo_gpio\n", __func__);
		ret = gpio_request(ts->pdata->power_ldo_gpio, "gt9xx_ldo_gpio");
		if (ret) {
			printk("gt9xx power ldo gpio request failed\n");
			return ret;
		}
		ret = gpio_direction_output(ts->pdata->power_ldo_gpio, 1);
		if (ret) {
...
free_ldo_gpio:	
	if (gpio_is_valid(ts->pdata->power_ldo_gpio))
		gpio_free(ts->pdata->power_ldo_gpio);
...
static int goodix_power_on(struct goodix_ts_data *ts)
{
	...
	{
		printk("%s, power_ldo_gpio\n", __func__);
		gpio_set_value(ts->pdata->power_ldo_gpio, 1);	
	}
	else
	{
		printk("%s, regulator\n", __func__);
			...
			struct goodix_ts_platform_data *pdata)
{
    ...
    /* power ldo gpio info*/
	pdata->power_ldo_gpio = of_get_named_gpio_flags(np, "goodix,power_ldo-gpio",
...
#endif
#ifdef SUPPORT_READ_TP_VERSION
#if defined(CONFIG_L6140_COMMON)		// xuke @ 20140811
	init_tp_fm_info(version_info, (char *)pdata->product_id, "Goodix");
#else
	//if (g_sensor_id >= 0x06 || g_sensor_id < 0)
		//printk("wxun: %s: Wrong tp sensor id(=%d)!!!!!!\n", __func__, g_sensor_id);
	//else if(0 == g_sensor_id)
		init_tp_fm_info(version_info, NULL, "[Goodix]");
	//else
		//init_tp_fm_info(version_info, NULL, "******");
#endif
#endif
#if GTP_ESD_PROTECT
    gtp_esd_switch(client, SWITCH_ON);
